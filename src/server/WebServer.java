...
class Worker extends WebServer implements HttpConstants, Runnable {
    ...
    private void handleLength(PrintStream ps, File targ) {
        long length = targ.length();

        try {
            if (isJNLP(targ)) {
                log.println("GOT JNLP FILE");
                toSent = readFileAsString(targ);
                toSent = toSent.replace("$$codebase",
                        "http://" + s.getLocalAddress().getHostAddress() + ":" + s.getLocalPort()
                        + findRelativePath(root, targ.getParentFile()).substring(2)).replace(File.separatorChar, '/');
                length = toSent.getBytes().length;
            }
    ...
    public static String findRelativePath(File base, File path) {
        return findRelativePath(base.getAbsolutePath(), path.getAbsolutePath());
    }
    ...
    void sendFile(File targ, PrintStream ps) throws IOException {
        ...
        if (isJNLP(targ)) {
            is.close();
            is = new BufferedInputStream(new ByteArrayInputStream(toSent.getBytes()));
        }
        try {
            int n;
    ...
    private boolean isJNLP(File targ) {
        /////////////////////////////////////////////////////////////////////
        String name = targ.getName();
        int ind = name.lastIndexOf('.');
        String ct = null;
        ...
        if (ct == null)
            return false;
        return ct.equals("application/x-java-jnlp-file");
    }
